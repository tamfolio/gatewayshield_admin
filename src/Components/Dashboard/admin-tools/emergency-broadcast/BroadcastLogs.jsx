...
export default function BroadcastLogs({ onEdit, refreshTrigger = 0 }) {
  ...
  const loadBroadcasts = useCallback(async () => {
    try {
      ...
      const result = await broadcastApi.getAll(
        apiClient,
        currentPage,
        10,
        apiFilters
      ...
      if (result) {
        if (Array.isArray(result)) {
          ...
          if (Array.isArray(result.data)) {
            ...
            result.data.broadcasts.data &&
            Array.isArray(result.data.broadcasts.data)
          ) {
            broadcastsArray = result.data.broadcasts.data;

            if (result.data.broadcasts.pagination) {
              totalPagesResult =
                result.data.broadcasts.pagination.totalPages || 1;
              totalCountResult =
                result.data.broadcasts.pagination.total ||
                broadcastsArray.length;
            }
            ...
            if (result.data.pagination) {
              ...
              totalCountResult =
                result.data.pagination.total || broadcastsArray.length;
            }
          } else if (
            result.data.broadcasts &&
            Array.isArray(result.data.broadcasts)
          ) {
      ...
      if (searchTerm.trim()) {
        broadcastsArray = broadcastsArray.filter(
          (broadcast) =>
      ...
      if (dateFilter) {
        ...
        broadcastsArray = broadcastsArray.filter((broadcast) => {
          const broadcastDate = new Date(
            broadcast.dateCreated || broadcast.createdAt
          ).toDateString();
                          ...
                          {loadingAlertTypes ? (
                            ...
                            alertTypes.map((type) => (
                              <button
                                key={type}
                                onClick={() =>
                                  addFilter("alertType", type, `Alert: ${type}`)
                                }
